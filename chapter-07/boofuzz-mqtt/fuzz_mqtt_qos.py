...
with s_block("FixedHeader"):
    s_bit_field(
        #value=0b00110000,
        value=0b00110010, # QOS: At least once delivery
        #value=0b00110100, # QOS: Exactly once delivery
        #value=0b00110110, # QOS: Reserved - must not be used
        #value=0b00111000, # DUP
        #value=0b00110001, # RETAIN
        width=8,
        fuzzable=False,
        name="ControlPacketTypeAndFlags"
    )
    ...
    with s_block("Remaining"):
        with s_block("VariableHeader"):
            ...
            with s_block("PacketIdentifier"):
                s_bytes(value=b"AA", fuzzable=True)
            with s_block("Properties"):
