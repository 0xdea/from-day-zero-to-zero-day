...
for d in "$srcdir"/test/test-data/2*/*.dwg "$srcdir"/test/test-data/r*/*.dwg; do
    ...
    echo "$d" || exit
    timeout 30 ./libtool --mode=execute programs/dwgread -v4 "$d" 2>"$log"
...
for d in "$srcdir"/test/test-big/2*/*.dwg; do
    ...
    log="$(basename "$d" .dwg)_$dir.log"
    echo "$d" || exit
    timeout 30 ./libtool --mode=execute programs/dwgread -v3 "$d" 2>"$log"
    #(sleep 10s; kill %1 2>/dev/null) &
done
...
if test -d "$srcdir/test/test-old"; then
    ...
    for d in r* 20*; do
        ...
        for f in */*.dwg; do
            ...
            timeout 30 "$abs_builddir/libtool" --mode=execute "$abs_builddir/programs/dwgread" -v4 "$f" 2>"$abs_builddir/$log"
        done
        cd .. || exit
    done
