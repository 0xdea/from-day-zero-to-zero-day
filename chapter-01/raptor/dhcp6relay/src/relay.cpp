...
void relay_forward(uint8_t *buffer, const struct dhcpv6_msg *msg, uint16_t msg_length) {
    ...
    memcpy(buffer, &option, sizeof(struct dhcpv6_option)); // KKK
    memcpy(buffer + sizeof(struct dhcpv6_option), msg, msg_length); // VULN? check callers
}
...
void relay_client(int sock, const uint8_t *msg, int32_t len, const ip6_hdr *ip_hdr, const ether_header *ether_hdr, relay_config *config) {    
    ...
    memcpy(&new_message.peer_address, &ip_hdr->ip6_src, sizeof(in6_addr)); // KKK
    new_message.hop_count = 0;

    memcpy(&new_message.link_address, &config->link_address.sin6_addr, sizeof(in6_addr)); // KKK
    memcpy(current_buffer_position, &new_message, sizeof(dhcpv6_relay_msg)); // KKK
    ...
    if(config->is_option_79) {
        ...
        memcpy(current_buffer_position, &option79, sizeof(linklayer_addr_option)); // KKK
        current_buffer_position += sizeof(linklayer_addr_option);

        memcpy(current_buffer_position, &ether_hdr->ether_shost, sizeof(ether_hdr->ether_shost)); // KKK
 ...
 */
 void relay_relay_reply(int sock, const uint8_t *msg, int32_t len, relay_config *config) {
    ...
    while ((current_position - msg) != len) {
        auto option = parse_dhcpv6_opt(current_position, &tmp); // [3] - condition
        current_position = tmp;
        switch (ntohs(option->option_code)) {
            case OPTION_RELAY_MSG: // condition
                memcpy(current_buffer_position, ((uint8_t *)option) + sizeof(struct dhcpv6_option), ntohs(option->option_length)); // VULN [4] - sink
                current_buffer_position += ntohs(option->option_length); // [5]
    ...
    memcpy(&target_addr.sin6_addr, &dhcp_relay_header->peer_address, sizeof(struct in6_addr)); // KKK [6]
...
void callback(evutil_socket_t fd, short event, void *arg) {
    ...
    int32_t len = recv(config->filter, message_buffer, 4096, 0); // KKK
    if (len <= 0) {
 ...
 * 
 * @brief               callback for libevent that is called everytime data is received at the server socket [1]
 *
 * @param fd            filter socket
 * @param event         libevent triggered event  
 * @param arg           callback argument provided by user
...
void server_callback(evutil_socket_t fd, short event, void *arg) {
    ...
    if ((data = recvfrom(config->local_sock, message_buffer, 4096, 0, (sockaddr *)&from, &len)) == -1) { // KKK [2] - source
...
void loop_relay(std::vector<relay_config> *vlans, swss::DBConnector *db) {
    ...
    for(relay_config &vlan : *vlans) {
        ...
        filter = sock_open(index, &ether_relay_fprog); // KKK
